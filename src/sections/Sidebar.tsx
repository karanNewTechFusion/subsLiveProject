  ...
  return (
    <>
      ...
      <div
        ...
        } ${collapsed ? "w-16" : "w-64"} lg:translate-x-0 lg:static flex flex-col`}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-white" style={{ paddingRight: '0px' }}>
          <div className={`flex items-center gap-3 ${collapsed ? "justify-center" : ""}`}>
            <div className={`flex items-center justify-center ${collapsed ? "w-8 h-8" : "w-32 h-8"}`}>
              <img
                src={collapsed ? imagePath.subsLogoMobile : imagePath.subsLogo}
                alt="SUBS Logo"
                className="w-full h-full object-contain"
        ...
        <nav className="flex-1 overflow-y-auto">
          <div className="space-y-1">
            {menuItems.map((item, index) => (
              ...
              >
                <div className={`p-1 rounded-md ${location.pathname === item.path ? "text-white" : "text-gray-900 group-hover:text-blue-500"}`}>
                  {item.icon}
                </div>
        ...
        {/* Logout */}
        <div className="p-3 border-t border-gray-200 bg-gray-50">
          <button className={`w-full group flex items-center gap-3 px-2 py-1.5 text-gray-700 hover:bg-red-50 hover:text-red-600 rounded-lg transition-all duration-200 ${collapsed ? "justify-center" : ""}`}>
            <div className="p-1 rounded-md text-gray-500 group-hover:text-red-500 relative">
              <Logout />
              {collapsed && (
                <div className="absolute left-8 bg-gray-800 text-white text-xs px-2 py-1 rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-50">
